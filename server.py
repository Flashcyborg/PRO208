...
def connectClient(message,client,client_name):
    ...
    if enterClientName in clients:
        if (not clients[client_name]["connected_with"]):
            clients[enterClientName]["connected_with"]=client_name
            clients[client_name]["connected_with"]=enterClientName
            otherClientsSocket = clients[enterClientName]["client"]
            greetMessage = f"Hello !,{enterClientName},{client_name} connected with you !"
            otherClientsSocket.send(greetMessage.encode())
            msg = f"Your succesfully connected with {enterClientName}!"
...
def disconnectClient(message,client,client_name):
    ...
    if enterClientName in clients:
        ...
        otherClientsSocket = clients[enterClientName]["client"]
        greetMessage = f"Hello !{enterClientName} , You have successfully disconnected with {client_name}"
        otherClientsSocket.send(greetMessage.encode())
        msg = f"You have succesfully discconnected with{enterClientName}!"
        client.send(msg.encode())
